<template>
  <div>
    ...
    <div class="Footer">
      <button type="submit" class="review-Scores-btn" v-on:click="updateKumrong">ขอทบทวนคะแนน</button>
      <button href="" v-on:click="sendTeacher(filteredCourse.teacher_name)" class="Teacher-contact">ติดต่ออาจารย์</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getCurrentTime(){
      ...
      const today = dateTime.toJSON().slice(0,10).replace(/-/g,'/');
      console.log('Today:',today);
      return today;
    },
    sendTeacher(teacherName){
      this.$router.push({name:'Profile',query: { teacherName : teacherName }});
      },
    async updateKumrong() {
      console.log("courseName:",this.courseName);
      ...
      try {
        const response = await axios.put("http://localhost:3000/api/courses/students/kumrong", {    
        courseName: this.courseName,
        user_Id: this.user_Id,
        date: this.Today,
        kumrong: this.kumrong
      });
      console.log('Today',this.Today)
      console.log('Kumrong updated successfully');
      alert('ส่งคำขอทบทวนคะแนนสำเร็จ');
      } catch (error) {
   
      console.error('Error updating kumrong:', error.response?.data || error.message);
      alert('Failed to update kumrong');
  }
},
  ...
  mounted() {
    ...
    if (coursesData) {           
      ...
      this.findCourse();
      if (this.filteredCourse) {
        this.user_Id = this.filteredCourse.student_scores.student_id
